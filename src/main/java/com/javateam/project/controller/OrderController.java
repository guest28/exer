...
public class OrderController {
	...
	public String orderInsert(@RequestParam(value = "cseq", defaultValue = "0") int[] cseqs, 
		...
		CartVO cartVO;
		List<CartVO> cartList = new ArrayList<>();	//db에서 새로 따오는 것이 아닌 이미 따온 것을 통째로 넘겨 받기
		...
		return "redirect:/order/order_list?oseq="+maxOseq;
...
//		String id = memberVO.getId();
		...
		for(OrderVO orderVO : orderList){
			totalPrice += orderVO.getPrice1() * orderVO.getQuantity();
		}
