		...
		<table id="orderList" class="table">
			...
			<c:forEach items="${orderList}" var="orderVO">
				<tr>
									...
									<div class="col-8">
										<div class="custom-control custom-checkbox">
											...
											<input type="checkbox" class="custom-control-input" checked="checked" disabled="disabled">
										 	<label class="custom-control-label mt-0">처리완료</label>  
										</div>
									</div>
								</div>
					        </c:otherwise>
					...
					<td style="width:80px">${orderVO.mname}</td>
					<td>${orderVO.pname}</td>
					<td style="width:50px">${orderVO.quantity}</td>
					<td style="width:100px">${orderVO.zipNum}</td>
					<td>
						${orderVO.address1}&nbsp;
						${orderVO.address2} 
					</td>
